...
<?php 
	...
	if (isset($_POST['btnOK'])==true) {
		if($qt->Them_tintuc()==1)
		{
		    echo '<script language="javascript">alert(\'Tin tức đã được thêm thành công!\')</script>';
		    header("location:index.php?p=tintuc&tt=1");
		}
    }
	if(mysql_num_rows(mysql_query('SELECT ThuTu FROM `tintuc`'))==0)
		   $max_tt=1;
	else
	{
		$result_max=mysql_query('SELECT max(ThuTu) FROM `tintuc`');
...
<tr><td colspan=3><b>Tóm Tắt</b><textarea name="tomTat" id="tomTat" class="txt" /></textarea> 
<script type="text/javascript">
...
<tr><td colspan=3><b>Nội Dung</b><textarea name="noiDung" id="noiDung" class="txt" /></textarea> 
<script type="text/javascript">
