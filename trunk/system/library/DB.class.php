...
class DB
{
	...
	static function countsub()
	{
		$query="select count(email) from subscribe";
		$get=self::GetQueryResult($query);
		return $get['count(email)'];
	...
	static public function GetOneRow($table,$condition,$select='*',$order='',$size='',$offset='')
	{
		if(is_array($select)){
		...
		$result=self::LimitQuery($table, array(
					'condition' => $condition,
					'select'=>$select,
					'size'=>$size,
					'offset'=>$offset,
	...
	static public function CheckIsset($table,$Condition) {
		$Sql="select * from $table where $Condition";
		$Query=self::GetQueryResult($Sql,0);
		if(empty($Query))
		{
			return 0;
		}
	...
	static public function LimitQuery($table, $options=array())
	{
		...
		$groupby = isset($options['groupby']) ?'group by '.$options['groupby'] : null;
		$cache = isset($options['cache'])?abs(intval($options['cache'])):0;
		$having=isset($options['having']) ?'having '.$options['having'] : null;
		...
		$sql = "SELECT {$select} FROM `$table` $condition $groupby $having $order $limitation ";
		return self::GetQueryResult( $sql, $one, $cache);
	}

	static public function LimitJoinQuery($table1,$table2, $options=array())
	{
		$condition = isset($options['condition']) ? $options['condition'] : null;
		$one = isset($options['one']) ? $options['one'] : false;
		$offset = isset($options['offset']) ? abs(intval($options['offset'])) : 0;
		...
		$select = isset($options['select']) ? $options['select'] : '*';
		$typejoin = isset($options['join']) ? $options['join'] : 'inner join';
		if(is_array($options['on']))
		{
			$on=implode(',',$options['on']);
		}
		$on = isset($options['on']) ? $options['on'] : "{$on}";
		$order = isset($options['order']) ? $options['order'] : null;
		$cache = isset($options['cache'])?abs(intval($options['cache'])):0;
		...
		$limitation = $size ? "LIMIT $offset,$size" : null;
		$groupby = isset($options['groupby']) ?'group by '.$options['groupby'] : null;
		$sql="SELECT {$select} FROM `$table1` $typejoin `$table2`
ON $on $condition $groupby $order $limitation";
		return self::GetQueryResult( $sql, $one, $cache);
	}

	static public function GetTopRecore($table,$column)
	{

		$sql="SELECT MAX($column) from $table";
		$get=self::GetQueryResult($sql);
		return $get['max(date_id)'];
	}

	static public function LimitSql($sql, $options=array())
	{
		//$condition = isset($options['condition']) ? $options['condition'] : null;
		$one = isset($options['one']) ? $options['one'] : false;
		$offset = isset($options['offset']) ? abs(intval($options['offset'])) : 0;
...
static public function Insert_Topup_Card_Detail( $table, $condition )
	{
		self::Instance();
	...
	static public function InsertNumberAsVarchar( $table, $condition )
	{
		self::Instance();
	...
	static public function UpdateSaleOffOrder( $table=null, $id=1, $updaterow=array(), $pkname='id' )
	{
	...
	static public function BuildCondition($condition=array(), $logic='AND')
	{
		...
		foreach ( $condition as $k => $v )
		{
			...
			$content .= " $logic $k $v_connect $v_str ";//and 'id' = 12
		}
