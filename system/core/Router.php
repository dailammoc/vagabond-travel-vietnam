...
class HKL_Router {
	...
	function LoadZRouter($request) {
		...
		//echo APPPATH.'config/routes.php';
	...
	{
		$this->routes['404_override']='error';
		if (count($segments) == 0)
		{
			return $segments;
		}
		...
		if (is_dir(APPPATH.'controllers/'.$segments[0]))
		{
			...
			if (count($segments) > 0)
			{
				...
				if ( ! file_exists(APPPATH.'controllers/'.$this->fetch_directory().$segments[0].'.php'))
				{
					...
					{
						$this->show_404($this->fetch_directory().$segments[0]);
					}
				}
			}
			else
