...
class Utility 
{
	...
	static function SplitStringHttp($string) {
		if(strpos($string, '/')){			
			$string = strtok($string, '/');
			$string = strtok('/');
		}
		//return  $string;
		$symbol = array('www.','.com','.vn','.net','.info','forum.','deal.','.org');
		foreach ($symbol as $symbol_record){
			if(strpos($string,$symbol_record )>=0)				
				$string = str_replace($symbol_record, '', $string);	
		}
		if($string=='bitvn')
			$string = 'bitvnorg';			
		return  $string;
